...
Wrap your app with `FormoAnalyticsProvider`. The integration is wallet-library agnostic; this example uses `@solana/wallet-adapter-react`:
...
Use manual events only with `solana: false` or a wallet that does not implement Wallet Standard. Call `formo.connect()` and `formo.disconnect()` when wallet state changes.
...
<AccordionGroup>
  ...
  <Accordion title="Why are my Solana wallet connections not showing up?">
    Use `@formo/analytics` 1.39.0 or later. Earlier versions require a [framework-kit](#solana-with-framework-kit) store. Also check that discovery is enabled and `solana.cluster` matches your chain. See [Solana manual wallet events](#solana-manual-wallet-events).
