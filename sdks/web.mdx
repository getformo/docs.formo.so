...
- [Wagmi](#wagmi) is recommended for EVM apps
- [Solana](#solana-integration) for Solana apps
...
Note that transaction and signature autocapture requires the use of [wagmi React hooks](#wagmi-integration). If your app also sends imperatively through viem (`walletClient.sendTransaction`, `.signMessage`, `.writeContract`), those calls create no wagmi mutation and are not captured by default; opt in with `wagmi: { eip1193Fallback: true }` (SDK 1.38.0+) and the SDK instruments the active connector's provider with the same request wrapper the default mode uses. Hook-driven calls are never double-counted. The flag is off by default so wagmi mode never touches the signing transport without an explicit, auditable decision.
</Note>
...
To make a business-critical event safe to retry, pass the reserved [`idempotency_key`](/data/events/track#deduplicating-events) property.
...
<CardGroup>
  ...
  <Card
    title="Tempo"
    icon="file-code"
    href="https://github.com/getformo/examples/tree/main/with-tempo"
  >
    examples/with-tempo
  </Card>
  <Card
...
The SDK skips tracking on localhost by default. 
...
const analytics = await FormoAnalytics.init('your-write-key', {
  tracking: {
    ...
    excludeTimezones: ['Asia/Bangkok', 'America/New_York'] // Exclude visitors whose browser timezone matches
  }
});
```
...
|--------|------|---------|-------------|
| `excludeHosts` | `string[]` | `[]` | Exact hostnames (`window.location.hostname`) to exclude from tracking. |
| `excludePaths` | `string[]` | `[]` | Exact paths (`window.location.pathname`) to exclude from tracking. |
| `excludeChains` | `ChainID[]` | `[]` | Chain IDs to exclude from tracking. |
| `excludeTimezones` | `string[]` | `[]` | IANA timezone names (e.g. `Asia/Bangkok`) to opt out of tracking entirely. |
| `excludeQueryParams` | `string[]` | `[]` | Query parameter names to strip from captured URLs before any event is sent. |

#### Excluding by timezone

When the visitor's browser-resolved timezone (via `Intl.DateTimeFormat().resolvedOptions().timeZone`) matches an entry in `excludeTimezones`, the SDK suppresses tracking entirely for that visitor.
...
Session replay records what visitors see and do on your app, so you can watch a session back from a user's profile. The SDK records the page with [rrweb](https://www.rrweb.io/) and sends the recording in small compressed chunks. Replay is off by default.

The recorder is not part of the main SDK file, so sites that do not use replay do not download it.

For browser installations, turn it on with `replay: true`. The SDK then loads its replay file (about 26 KB) only when recording starts. It comes from jsDelivr, or from unpkg if jsDelivr fails, is pinned to your SDK version, and is checked with subresource integrity:

```html
<script
  src="https://cdn.formo.so/analytics@latest"
  defer
  onload="window.formofy('<YOUR_WRITE_KEY>', { replay: true });"
></script>
```

For npm installations, install `@rrweb/record` and use `replay()` from `@formo/analytics/replay`. The recorder is then bundled with your app and no script is loaded at runtime. Apps that do not import `@formo/analytics/replay` do not include any replay code:
...
<FormoAnalyticsProvider
    ...
    options={{
        replay: replay({
            record,
            sampleRate: 0.5, // record half of the tabs (default: 1)
        }),
    }}
>
```

You can also pass `replay: true` from npm. The SDK then loads the replay file from the CDN, as for browser installations.

| Option | Default | Description |
|---|---|---|
| `sampleRate` | `1` | Share of browser tabs to record, from 0 to 1. |
| `record` | | rrweb's `record` function, passed to `replay()` from `@formo/analytics/replay`. |
| `scriptUrl` | jsDelivr, then unpkg | Load the replay file (`dist/replay.umd.min.js` from the npm package) from your own origin instead. |
| `maskAllInputs` | `true` | Mask the value of every input, textarea and select. |
| `maskTextSelector` | | CSS selector whose text is masked. |
| `blockSelector` | | CSS selector whose elements are not recorded. |

**Privacy.** Input values are always masked unless you set `maskAllInputs: false`, and password fields are always masked. Mark other sensitive content in your markup:

```html
<span data-formo-mask>1,234.56 ETH</span>   <!-- text is replaced with * -->
<div data-formo-block>...</div>             <!-- not recorded, shown as an empty box -->
```

Canvas and fonts are not recorded. The page URL in a recording follows the same query parameter rules as your events (see `excludeQueryParams`). Replay follows consent: `optOutTracking()` stops the recording and drops what has not been sent, and pages excluded with `excludePaths` or `excludeHosts` are not recorded.

**Sessions.** Each browser tab records its own replay. A replay continues across page loads in the same tab, and a new one starts after 30 minutes without input, or when you call `reset()`. While a replay is recording, every event carries its `replay_id`, so you can jump from an event to the moment it happened.

**Content security policy.** If your site sets a CSP and loads the replay file from the CDN, allow `https://cdn.jsdelivr.net` and `https://unpkg.com` in `script-src`, or host the file yourself with `scriptUrl`. Replay chunks go to the same endpoint as your events, so the [proxy](#proxy) setup applies to them too.
...
- **[framework-kit](#solana-with-framework-kit) (Recommended)**: pass its store for wallet, cluster, and recorded transaction lifecycle events.
- **[Solana Kit](#solana-kit)**: wallet `detect`, `connect`, and `disconnect` are autocaptured through Wallet Standard without a store.
...
For Solana dapps, use framework-kit (`@solana/client`) and pass `client.store`. Wallet events hand off to the store without double-counting, while the store adds cluster switches and transactions recorded in it. React transaction hooks do not write to the store and still need [explicit tracking](#manually-tracking-solana-transactions-and-signatures).

See the focused [framework-kit example](https://github.com/getformo/examples/tree/main/with-solana-framework-kit).
...
<Tabs>
  <Tab title="React">
    Pass `client.store` in the Solana options. This adds network changes and transactions recorded in the store; hook-based transactions still need explicit tracking.
...
The following events are autocaptured via zustand store subscription:
...
| Transaction | Started, broadcasted, confirmed, reverted, or rejected, for transactions framework-kit records in the store (see the note on hooks [below](#manually-tracking-solana-transactions-and-signatures)) |

Wallet `detect` events still come from Wallet Standard discovery.

#### Solana Kit

If your app uses Solana Kit directly, give Formo the same cluster. Formo observes Wallet Standard independently, so it does not need the Kit client or a store.
  ...
  return (
    <ClientProvider client={client}>
      <FormoAnalyticsProvider
        writeKey="<YOUR_WRITE_KEY>"
        options={{
          evm: false, // Optional for a Solana-only app
...
See the complete [Solana Kit example](https://github.com/getformo/examples/tree/main/with-solana).

#### Other Wallet Standard integrations

The SDK observes [Wallet Standard](https://github.com/wallet-standard/wallet-standard) account changes without wrapping wallet methods or adding a Solana dependency.

```bash
npm install @formo/analytics --save
```

This `@solana/wallet-adapter-react` example needs only `FormoAnalyticsProvider`. Discovery works whenever the connected wallet registers with the page through Wallet Standard:
  ...
  return (
    <ConnectionProvider endpoint={endpoint}>
      <WalletProvider wallets={[]} autoConnect>
        <FormoAnalyticsProvider
          writeKey="<YOUR_WRITE_KEY>"
          options={{ evm: false }} // Optional: disable EVM tracking for Solana-only apps
        >
          <YourApp />
        </FormoAnalyticsProvider>
      </WalletProvider>
...
| Event | Trigger |
|-------|---------|
| Detect | A wallet registers with the page |
| Connect | The wallet authorizes an account, including a session restored on page load |
| Disconnect | The wallet revokes its accounts |
| Chain change | `formo.solana.setCluster()` is called while a wallet is connected |

**Cluster**

Wallet Standard does not expose your app's active cluster, only the clusters a wallet supports. Formo reports `mainnet-beta` when the wallet supports it, and otherwise the first Solana cluster the wallet lists. Set non-mainnet apps explicitly:

```tsx
options={{ evm: false, solana: { cluster: 'devnet' } }}
```

For runtime switches, call `formo.solana.setCluster('devnet')`. This also emits a `chain` event. See [manual transaction and signature tracking](#manually-tracking-solana-transactions-and-signatures).

#### Wallets without Wallet Standard

Most Solana wallets require no manual connection tracking. If a custom or legacy wallet does not publish Wallet Standard account changes, call `formo.connect()` and `formo.disconnect()` when its state changes.

<Warning>
Use `formo.connect()` for wallet connections. `formo.track('Connect', ...)` creates a custom event and does not populate wallet connection analytics.
</Warning>
...
const chainId = SOLANA_CHAIN_IDS['mainnet-beta'];

formo.connect({ chainId, address }, { providerName });
formo.disconnect({ chainId, address });
```
...
Mainnet has several spellings: Solana Kit's wallet plugin takes `solana:mainnet`, framework-kit's client takes `mainnet`, and Formo uses `mainnet-beta`. Map between them when deriving one from another.
...
<Note>
Using Wagmi (RainbowKit, ConnectKit, AppKit, Privy, and most React dapps)? WalletConnect is already tracked through the [Wagmi integration](#wagmi); skip this section. `registerProvider` refuses in Wagmi mode to prevent double-reporting.
</Note>

The SDK discovers wallets that announce themselves ([EIP-6963](https://eips.ethereum.org/EIPS/eip-6963)) or sit at `window.ethereum`, which covers injected browser wallets only. A WalletConnect provider is different: your app constructs it in code with `EthereumProvider.init(...)`, so the SDK cannot find it on its own. Hand it over with `registerProvider` (SDK 1.38.0+):

```tsx
import { EthereumProvider } from '@walletconnect/ethereum-provider';

const provider = await EthereumProvider.init({ projectId, chains: [1] });
...
- Returns `true` when capture works; `false` when it refuses (Wagmi mode, EVM tracking disabled, or not a valid EIP-1193 provider).
- The same call works for any other provider your app constructs rather than discovers; pass `{ name, rdns }` as a second argument to label a custom one.

See the [with-walletconnect example](https://github.com/getformo/examples/tree/main/with-walletconnect) for a complete integration.
...
| cdn.formo.so | Loads the HTML snippet (only for [Website installation](#html-snippet)) e.g. `https://cdn.formo.so/analytics@latest` |
...
If you are using the HTML / website snippet, replace the install script with:
...
<AccordionGroup>
  ...
  <Accordion title="Do I need framework-kit to use Formo on Solana?">
    framework-kit is recommended for Solana dapps because its store supplies cluster switches and recorded transaction lifecycle events. It is not required: Wallet Standard still autocaptures `detect`, `connect`, and `disconnect` for compatible wallets used through Solana Kit or other integrations. Signatures and transactions outside the store require explicit tracking.
  </Accordion>
</AccordionGroup>
