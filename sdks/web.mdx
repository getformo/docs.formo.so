...
The Web SDK can measure the [Core Web Vitals](https://web.dev/articles/vitals) from real visitors: LCP, INP, CLS, FCP, and TTFB. It uses Google's [`web-vitals`](https://github.com/GoogleChrome/web-vitals) library. The Overview page shows each metric as a percentile (P90 by default; P50, P75 and P99 available), its trend over time, and a breakdown by page, device, and country.
...
```tsx Next.js
'use client'; // import web-vitals in the client component that builds options
...
export function Providers({ children }: { children: React.ReactNode }) {
  return (
    <FormoAnalyticsProvider writeKey="<YOUR_WRITE_KEY>" options={{ webVitals }}>
      {children}
    </FormoAnalyticsProvider>
  );
}
