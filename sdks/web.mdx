...
- **[Any wallet library](#solana-wallet-discovery)**: initialize the SDK and wallet events are autocaptured.
- **[framework-kit](#solana-with-framework-kit)** (`@solana/client` + `@solana/react-hooks`): additionally pass `client.store` and the transaction lifecycle and cluster switches are autocaptured too.
...
<Note>
Wallet Standard discovery requires `@formo/analytics` 1.39.0 or later. On earlier versions, Solana wallet events are only autocaptured with a framework-kit store.
</Note>
