...
<Note>
If you're already using [Wagmi](https://wagmi.sh), the Formo SDK can connect directly. This is the recommended way to install Formo for apps.
</Note>
...
<Tip>
**Why use Wagmi integration?** When `wagmi` options are provided, the SDK hooks directly into Wagmi's state management instead of wrapping EIP-1193 providers. This provides:
...
<Warning>
**Important:** Always provide the `queryClient` to enable full event tracking. Without it, signature and transaction events will not be captured.
</Warning>
...
<Note>
Use [Wagmi](#wagmi) instead for better event tracking and compatibility.
</Note>
