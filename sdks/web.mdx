...
Call `reset()` on a real logout or disconnect in an integration where the SDK cannot see the disconnect itself, for example a custom connector where you call `connect()` and `disconnect()` yourself. In Wagmi mode the SDK observes the disconnect and clears the wallet identity on its own, so you rarely need it.
