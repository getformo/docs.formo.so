...
- **[framework-kit](#solana-with-framework-kit) (Recommended)**: autocaptures wallet connections, cluster switches, and transactions recorded in its store.
- **[Wallet Standard](#solana-wallet-discovery)**: autocaptures wallet `detect`, `connect`, and `disconnect` for compatible wallets, regardless of library.

<Note>
Wallet Standard discovery requires `@formo/analytics` 1.39.0 or later. Earlier versions only autocapture Solana connections from a framework-kit store.
</Note>

#### Solana with framework-kit

Passing a framework-kit store adds cluster switches and transactions recorded in that store. Wallet events hand off to the store without double-counting. React transaction hooks do not write to the store and still need [explicit tracking](#manually-tracking-solana-transactions-and-signatures).
...
Wrap your app with `FormoAnalyticsProvider`. This example uses `@solana/wallet-adapter-react`, but discovery works with any library connected to a Wallet Standard wallet:
...
Track Solana signatures explicitly. Track transactions too, except those recorded in a framework-kit store. Its React hooks (`useSolTransfer`, `useSendTransaction`, `useTransactionPool`) do not write to that store.
...
<AccordionGroup>
  ...
  <Accordion title="Why are my Solana wallet connections not showing up?">
    Use `@formo/analytics` 1.39.0 or later. Earlier versions require a [framework-kit](#solana-with-framework-kit) store. Also check that `solana.cluster` matches your chain. If the wallet does not implement Wallet Standard, use the [manual connection fallback](#wallets-without-wallet-standard).
  ...
  <Accordion title="Do I need framework-kit to use Formo on Solana?">
    No. Wallet Standard autocaptures `detect`, `connect`, and `disconnect`. framework-kit adds cluster switches and transactions recorded in its store. Signatures and other transactions require explicit tracking.
  </Accordion>
</AccordionGroup>
