...
For browser installations, turn it on with `replay: true`. The SDK then loads the rrweb recorder from jsDelivr, pinned to one version and checked with subresource integrity:
...
For npm installations, install `@rrweb/record` and pass its `record` function. The recorder is then bundled with your app and no script is loaded at runtime:
...
| `record` | | rrweb's `record` function from `@rrweb/record`. |
| `scriptUrl` | pinned jsDelivr URL | Load the recorder from your own origin instead. |
...
**Content security policy.** If your site sets a CSP and you use the CDN recorder, allow `https://cdn.jsdelivr.net` in `script-src`, or host the file yourself with `scriptUrl`. Replay chunks go to the same endpoint as your events, so the [proxy](#proxy) setup applies to them too.
