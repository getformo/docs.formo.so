...
Solana wallets are discovered through the [Wallet Standard](https://github.com/wallet-standard/wallet-standard), the Solana counterpart of EIP-6963. Every modern Solana wallet (Phantom, Solflare, Backpack, and the rest) announces itself through it, and every wallet library sits on top of it, so wallet `detect`, `connect`, and `disconnect` events are autocaptured for `@solana/wallet-adapter`, framework-kit, Privy, Dynamic, Reown, and custom connectors alike. No Solana configuration is needed.

- **[Any wallet library](#solana-wallet-discovery)**: wallet `detect`, `connect`, and `disconnect` are autocaptured.
- **[framework-kit](#solana-with-framework-kit)** (`@solana/client` + `@solana/react-hooks`): additionally pass `client.store` to autocapture cluster switches and transactions recorded in the store.
- **[Manual wallet events](#solana-manual-wallet-events)**: for `solana: false`, or a wallet that does not implement the Wallet Standard.

<Note>
Wallet Standard discovery requires `@formo/analytics` 1.39.0 or later. Earlier versions only autocapture Solana connections through a framework-kit store.
</Note>

#### Solana wallet discovery

The SDK announces itself to Wallet Standard wallets on the page and subscribes to each wallet's account changes as a read-only observer. It never wraps or intercepts wallet methods, and it does not depend on any Solana package.

```bash
npm install @formo/analytics --save
```

Wrap your app with `FormoAnalyticsProvider` anywhere in the tree. This example uses `@solana/wallet-adapter-react`, but nothing in it is specific to that library:
...
The Wallet Standard lists the clusters a wallet supports, not the one your app is on, so connections are reported on `mainnet-beta` unless you say otherwise. A devnet or testnet app should name its cluster:

```tsx
options={{ evm: false, solana: { cluster: 'devnet' } }}
```

or switch it at runtime with `formo.solana.setCluster('devnet')`, which also emits a `chain` event for the connected wallet. Transactions and signatures still need explicit [`transaction`](/data/events/transaction) and [`signature`](/data/events/signature) calls, described [below](#manually-tracking-solana-transactions-and-signatures).

To turn Solana discovery off entirely, pass `solana: false`.

#### Solana with framework-kit

With framework-kit, the SDK additionally subscribes to its zustand store as a read-only observer. The store adds cluster switches detected from the RPC endpoint and the transaction lifecycle for transactions recorded there. Wallet connects and disconnects are then reported from the store rather than from Wallet Standard discovery, so each connection is counted once. Framework-kit's React transaction hooks keep their state outside this store, so calls made through those hooks still need the explicit tracking shown [below](#manually-tracking-solana-transactions-and-signatures).
...
<Tabs>
  <Tab title="React (Recommended)">
    Wrap your app with `FormoAnalyticsProvider` and pass `client.store` in the solana options. The SDK automatically tracks wallet connects, disconnects, account switches, and network changes. It also tracks transactions recorded in the store; transactions sent through framework-kit's React hooks still need explicit calls.
...
#### Solana manual wallet events

If you pass `solana: false`, or your wallet does not implement the Wallet Standard, nothing is autocaptured. Call `formo.connect()` and `formo.disconnect()` at the points where your wallet state changes.
...
Initialize the SDK with `solana: false`, so discovery and your own calls never both report the same connection:

```tsx
<FormoAnalyticsProvider
  writeKey="<YOUR_WRITE_KEY>"
  options={{ evm: false, solana: false }}
>
  <YourApp />
</FormoAnalyticsProvider>
```
...
Signature events for Solana wallets must be tracked explicitly on every integration path, and transaction events on every path except what a framework-kit store records. On framework-kit this is because its React hooks (`useSolTransfer`, `useSendTransaction`, `useTransactionPool`) manage transaction state locally and don't write to the store.
...
<AccordionGroup>
  ...
  <Accordion title="Why are my Solana wallet connections not showing up?">
    Check the SDK version first. On `@formo/analytics` 1.39.0 or later, Solana wallet connections are autocaptured through the Wallet Standard with any wallet library, so upgrading is the fix. On earlier versions they were only autocaptured with a [framework-kit](#solana-with-framework-kit) store. If you passed `solana: false`, discovery is off and you emit them yourself; see [Solana manual wallet events](#solana-manual-wallet-events). Connections are reported on `mainnet-beta` unless you set `solana.cluster`, so check the chain filter too.
  ...
  <Accordion title="Do I need framework-kit to use Formo on Solana?">
    No. Wallet Standard discovery autocaptures `detect`, `connect`, and `disconnect` with any Solana wallet library. framework-kit additionally provides cluster switches and the lifecycle of transactions recorded in its store. Signatures and transactions outside that store require explicit `formo.signature()` and `formo.transaction()` calls.
  </Accordion>
</AccordionGroup>
