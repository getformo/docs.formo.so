...
The SDK can measure the [Core Web Vitals](/data/events/web-vitals) of each page load from real visitors: LCP, INP, CLS, FCP, and TTFB. It uses Google's [`web-vitals`](https://github.com/GoogleChrome/web-vitals) library, which you pass in, and sends one `web_vitals` event per page load when the visitor first leaves or hides the page. The Overview dashboard shows the 75th percentile of each metric and the slowest pages.
