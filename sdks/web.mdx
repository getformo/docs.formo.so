...
You do not need to configure anything to track wallet events.

To track custom events (in-app actions, key conversions) use the [`track`](/data/events/track) function with send details about the action:
...
The following table lists the two domains that are used by Formo to successfully load the SDK and send events to Formo:

| Domain | Use case |
|--------|----------|
| cdn.formo.so | Loads the SDK (only for [Website installation](#websites)) |
...
- The Formo SDK is also available on [unpkg](https://unpkg.com/) (`https://unpkg.com/@formo/analytics`)
- You can serve the Formo SDK on your own domain.
