...
You can configure which wallet events and [web vitals](#web-vitals) are automatically captured by the SDK:
...
The SDK measures the [Core Web Vitals](/data/events/web-vitals) of each page load from real visitors: LCP, INP, CLS, FCP, and TTFB. It sends one `web_vitals` event per page load, when the visitor first leaves or hides the page, and adds no extra script or dependency. The Overview dashboard shows the 75th percentile of each metric and the slowest pages.

Web vitals are on by default (SDK 1.42.0+). To turn them off:
...
`autocapture: false` also turns them off. Consent and [tracking exclusions](#environments) apply as for other events.
...
- A page loaded in a background tab reports only TTFB, because nothing was painted for the visitor.
