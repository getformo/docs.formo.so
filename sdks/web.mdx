...
- **[Solana Kit](#solana-kit-recommended) (Recommended)**: wallet `detect`, `connect`, and `disconnect` are autocaptured through Wallet Standard.
- **[framework-kit](#solana-with-framework-kit)**: existing `@solana/client` apps can additionally pass their store for cluster and transaction lifecycle events.
...
Install Formo alongside the current Solana Kit wallet stack:

```bash
npm install @formo/analytics @solana/kit @solana/kit-plugin-rpc @solana/kit-plugin-wallet @solana/react
```

Create a Kit client and give Formo the same cluster. Formo observes the wallet's Wallet Standard events independently, so it does not need the Kit client or a store:
...
If your app uses framework-kit (`@solana/client`), pass `client.store` to add cluster switches and transactions recorded in that store. Wallet events hand off to the store without double-counting. React transaction hooks do not write to the store and still need [explicit tracking](#manually-tracking-solana-transactions-and-signatures).
...
<AccordionGroup>
  ...
  <Accordion title="Do I need framework-kit to use Formo on Solana?">
    No. Wallet Standard autocaptures `detect`, `connect`, and `disconnect` for compatible wallets, including wallets used through Solana Kit. If your app already uses framework-kit, its store can additionally supply cluster switches and recorded transaction lifecycle events. Signatures and other transactions require explicit tracking.
  </Accordion>
</AccordionGroup>
