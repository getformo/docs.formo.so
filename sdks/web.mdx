...
By default, Formo scopes identity cookies to minimize data sharing across subdomains. With the default `crossSubdomainCookies` setting, cookies remain scoped to the current host, which reduces unintended cross-subdomain tracking.

| Value | Behavior |
|-------|----------|
| `false` (default) | Cookies are scoped to the current hostname only. A cookie set on `app.example.com` is **not** visible from `www.example.com`. |
| `true` | Cookies are set on the root domain (e.g. `.example.com`), sharing visitor identity across all subdomains. |

If your product surface spans multiple subdomains (e.g. `example.com` and `app.example.com`), we recommend enabling `crossSubdomainCookies` to ensure accurate visitor counts and consistent attribution across your subdomains.

When `crossSubdomainCookies` is enabled:
...
<script
  src="https://cdn.formo.so/analytics@latest"
  defer onload="window.formofy('WRITE_KEY', { crossSubdomainCookies: true });"
></script>
```
  </Tab>
...
The Formo Web SDK supports Solana. Solana tracking works alongside EVM tracking — you can track both EVM and Solana wallets in the same app.

You need to use the [React/Next.js (without Wagmi)](#react-%26-next-js-without-wagmi) installation method to enable tracking of Solana wallets.
