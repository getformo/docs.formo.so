...
The SDK automatically detects and extracts [ERC-8021](https://www.erc8021.com/) builder codes from transaction calldata. If your app appends builder codes to transactions, Formo will detect and include them in [transaction events](/data/events/transaction#builder-codes).

[Integrate Builder Codes into your app using Wagmi or Viem to attribute onchain activity.](https://docs.base.org/base-chain/builder-codes/app-developers)

No additional SDK configuration is required — builder code extraction happens automatically for all tracked transactions.

## Frequently Asked Questions

<AccordionGroup>
  <Accordion title="What is the difference between the Wagmi integration and the standard React integration?">
    The [Wagmi integration](#wagmi) automatically tracks wallet connections, disconnections, chain switches, transactions, and signatures by hooking into Wagmi's wallet adapter. The standard [React integration](#react--nextjs) tracks pageviews and sessions but requires manual calls to track wallet events. Use the Wagmi integration if your dApp uses Wagmi.
  </Accordion>
