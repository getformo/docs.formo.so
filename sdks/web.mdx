...
<Tabs>
  <Tab title="Init with store (Recommended)">
    Pass `client.store` from framework-kit during initialization. The SDK automatically tracks wallet connects, disconnects, account switches, and network changes.
    ...
    // Create the framework-kit client
    const client = createClient({
      cluster: 'devnet',
      walletConnectors: autoDiscover(),
    });

    // Initialize Formo with the client store
  ...
  <Tab title="Lazy setup">
    If the store isn't available at init time, set it later:

    ```tsx
    const formo = await FormoAnalytics.init('<YOUR_WRITE_KEY>');

    // Later, when the client is ready:
    formo.solana.setStore(client.store);
    ```
  </Tab>
</Tabs>

**Autocaptured events** (via store subscription):
...
```tsx
import { TransactionStatus, SignatureStatus, SOLANA_CHAIN_IDS } from '@formo/analytics';
