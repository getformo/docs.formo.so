...
This replaces the [HTML snippet](#html-snippet) above. Deferred scripts run in order, so `window.webVitals` is ready when the SDK's `onload` calls `formofy`. Pin an exact `web-vitals` version with its `integrity` hash (see [Subresource Integrity](/security/sri)); the hash above is for 6.2.2. With `FormoAnalyticsProvider`, pass it the same way: `options={{ webVitals }}`. Consent and [tracking exclusions](#environments) apply as for other events.
