...
| Reset | In Mobile SDK 1.2.0+, `reset()` starts a new session and keeps the anonymous id. Earlier versions remove it. |
...
// Forget the current user (user_id, wallet identity) and start a new session.
// Mobile SDK 1.2.0+ keeps the anonymous id.
formo.reset();
```

<Note>
Starting in Mobile SDK 1.2.0, `reset()` preserves the anonymous id. Earlier versions removed it; attribution was already preserved. Existing calls need no signature change, but logout flows will now keep the same device identity.
</Note>

Call `reset()` on a real application logout, not when the wallet address changes. For a manual wallet integration, call `disconnect()` when the wallet disconnects; Wagmi mode observes this automatically. If your app has no separate login identity, you can usually omit `reset()` entirely. When the user withdraws consent, use `optOutTracking()` to clear the anonymous id and attribution as well.
