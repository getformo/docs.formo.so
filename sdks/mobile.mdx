...
<Note>
Starting in Mobile SDK 1.2.0, `reset()` preserves the anonymous id and visit attribution. Earlier versions removed both. Existing calls need no signature change, but logout flows will now keep the same device identity.
</Note>
