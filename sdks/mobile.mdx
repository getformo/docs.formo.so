...
Call `reset()` on a real application logout, not when the wallet address changes. In Wagmi mode the SDK observes wallet disconnects itself. When the user withdraws consent, use `optOutTracking()` to clear the anonymous id and attribution as well.
