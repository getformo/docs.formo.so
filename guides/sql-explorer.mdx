...
Formo's SQL engine integrates with external BI tools via the **Analytics API**.
...
The same editor can also query [Dune](/integrations/dune) (on-chain data) instead of Formo. Prefix a table with `dune.` and the query runs on Dune (Trino SQL); everything else runs on Formo (ClickHouse SQL).
...
<Note>
  A single query can't mix Dune and Formo tables; they're separate
