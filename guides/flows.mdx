...
- Formo is installed and tracking page views and key events
- You have user events like connect wallet and transaction completion events
- Basic understanding of your onboarding funnel (signup > wallet connect > first transaction)
...
Saved flows refresh daily, so you can track onboarding improvements over time as you ship fixes.
...
- See [How to Build Conversion Funnels](/guides/conversion-funnel) to measure step-by-step conversion
- Use [SQL Explorer](/guides/sql-explorer) to query average time spent in each step
- Set up dashboards to track onboarding KPIs weekly
...
<Accordion title="Why is my flow showing circular paths (loops)?">
Loops mean users revisit pages. For example, users might visit Swap > Portfolio > Swap again. This is normal and shows exploration behavior. Filter to first-visit-only if you want linear paths.
</Accordion>
...
<Accordion title="How often do flows update?">
Flows update in real time as new user sessions are recorded. Historical flows (from past dates) refresh daily.
</Accordion>
