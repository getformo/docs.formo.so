...
- Formo installed and tracking sessions
- At least 7 days of data (for Day 7 retention)
- A product you're actively improving
...
<Tip>
**What's a good retention rate?** In crypto, benchmarks vary by product:
- **DEX**: Day 7 = 15-25%, Day 30 = 5-10%
- **Lending**: Day 7 = 25-40%, Day 30 = 10-20%
- **NFT marketplace**: Day 7 = 10-20%, Day 30 = 3-7%
- **Gaming**: Day 7 = 30-50%, Day 30 = 15-30%

Your goal: beat the benchmark. A 2-3% improvement is meaningful.
</Tip>
...
<Step>
Click **Save as Segment** (Growth+ plan required). Name it "Churned Users Q1 2026".
</Step>
...
## Part 6: Set Retention Alerts

Monitor retention health with alerts. If Day 7 retention drops below your threshold, get notified immediately:
...
<Step>
Set up: "Day 7 Retention drops below 20%". Choose notification method (email, Slack, webhook).
</Step>

<Step>
Save. You'll be alerted the moment it triggers.
</Step>
</Steps>
...
| App Type | Day 7 | Day 30 | Day 90 | Notes |
|----------|-------|--------|--------|-------|
| DEX | 15-25% | 5-10% | 2-4% | High volume, low commitment |
| Lending | 25-40% | 10-20% | 5-10% | Recurring value, sticky |
| NFT Marketplace | 10-20% | 3-7% | 1-3% | Collectible-driven, sparse |
| Gaming | 30-50% | 15-30% | 8-15% | Highly engaging, daily drivers |
| Staking | 35-50% | 15-25% | 8-15% | Lock-in effect, recurring value |

## Pro Tips for Improving Retention
...
SELECT
  ...
  countIf(last_seen < now() - interval '30 days') as churned_users,
  round(countIf(last_seen < now() - interval '30 days') / count(*) * 100, 2) as churn_rate_pct
FROM users
...
| New users stalling | 3 days, <5 sessions | In-app nudge + tutorial replay | 20-30% return |
...
- **[Create a Conversion Funnel](/guides/howto/conversion-funnel)**: High retention is good, but you need high conversion first.
- **[Use Segments for Re-engagement](/features/wallet-intelligence/segments)**: Build audiences of churned users for campaigns. (Growth+ plan)
- **[Build a Protocol Dashboard](/guides/howto/protocol-dashboard)**: Track retention alongside other key metrics in one view.
- **[Set Up Alerts](/features/product-analytics/alerts)**: Get notified of retention drops.
...
<Accordion title="Why does my Day 7 retention look low compared to benchmarks?">
Crypto apps typically have lower retention than traditional apps because:
1. **High user acquisition cost** attracts price-sensitive, low-LTV users
2. **Price volatility** causes churn (markets down, users disappear)
3. **Regulatory uncertainty** spooks users away
4. **App fatigue** (so many crypto apps, users bounce between them)

Your goal: beat your own baseline, not match a Fortune 500 SaaS.
</Accordion>

<Accordion title="Do I need a Growth+ plan to use segments?">
Yes. Segments are a Growth+ feature. Free and Starter plans can view retention and export user lists manually via CSV, but cannot save reusable segments.
</Accordion>

<Accordion title="How long does retention data take to update?">
Retention analysis runs on daily aggregates. Your retention metrics update once per day, typically in the morning (UTC). Real-time retention data is not available.
</Accordion>
...
**Ready to act?** Start with the Insights page this week. Pick one churn signal and fix it. You'll be surprised how fast retention improves.

Questions? Email us at support@formo.so or ask the chat bubble in your dashboard.