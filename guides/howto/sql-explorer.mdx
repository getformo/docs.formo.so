...
FROM users
WHERE argMaxMerge(last_utm_source) IS NOT NULL
GROUP BY utm_source
ORDER BY users DESC
```
...
Configuration details are at [/data/bi](/data/bi). You'll need your API key from [Workspace Settings > API Keys](/features/product-analytics/api-reference).
