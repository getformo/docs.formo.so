...
- `operand`: The field to filter on (e.g., `revenue`, `num_sessions`, `first_seen`, `last_seen`, `volume`, `points`, `location`, `device`, `browser`, `os`)
- `operator`: The comparison operator (`equals`, `greater`, `less`, `greaterOrEqual`, `lessOrEqual`, `in`)
...
```json
[
  {"operand": "revenue", "operator": "greater", "value": 100},
  {"operand": "num_sessions", "operator": "greaterOrEqual", "value": 5}
]
```
...
- `operand`: The field to filter on (e.g., `net_worth_usd`, `tx_count`, `first_onchain`, `last_onchain`)
- `operator`: The comparison operator (`equals`, `greater`, `less`, `greaterOrEqual`, `lessOrEqual`)
- `value`: The value to compare against
...
```json
[
  {"operand": "net_worth_usd", "operator": "greater", "value": 1000},
  {"operand": "tx_count", "operator": "greater", "value": 10}
]
```

### `socials`

JSON array of social field names to filter by. Only profiles that have a non-empty value for ALL specified social fields will be returned.

Supported fields: `ens`, `farcaster`, `twitter`, `discord`, `telegram`, `lens`, `email`, `github`, `linkedin`, `basenames`, `linea`, `website`, `instagram`, `facebook`, `tiktok`, `youtube`, `reddit`
...
Example - Find profiles with both ENS and Twitter:
...
JSON array of per-chain filter conditions for net worth. Each filter is an array with three elements:
- `[0]`: Chain ID (e.g., `"1"` for Ethereum, `"137"` for Polygon)
- `[1]`: Operator (`greater`, `less`, `greaterOrEqual`, `lessOrEqual`, `equals`)
- `[2]`: Value (net worth in USD)

Example - Find profiles with >$1000 net worth on Ethereum:

```json
[["1", "greater", 1000]]
```

Example - Find profiles with >$500 on both Ethereum and Polygon:

```json
[["1", "greater", 500], ["137", "greater", 500]]
```
...
curl -sS \
  ...
  --data-urlencode 'profile_filters=[{"operand":"net_worth_usd","operator":"greater","value":10000}]' \
  --data-urlencode 'filters=[{"operand":"num_sessions","operator":"greaterOrEqual","value":5}]'
```
...
curl -sS \
  -H "Authorization: Bearer <your_workspace_api_key>" \
  -G "https://api.formo.so/v0/profiles" \
  --data-urlencode 'socials=["ens"]' \
  --data-urlencode 'chain_filters=[["1","greater",0]]' \
  --data-urlencode 'expand=chains,labels'
```

### Find Users by Revenue and Sessions

```bash
curl -sS \
  -H "Authorization: Bearer <your_workspace_api_key>" \
  -G "https://api.formo.so/v0/profiles" \
  --data-urlencode 'filters=[{"operand":"revenue","operator":"greater","value":1000},{"operand":"num_sessions","operator":"greaterOrEqual","value":10}]'
```

<Note>
The `lifecycle` field is computed dynamically based on user activity patterns and cannot be used as a filter. To find users by lifecycle stage, you can filter by the underlying fields (`first_seen`, `last_seen`, `activity_dates`) or retrieve profiles and filter client-side.
</Note>
