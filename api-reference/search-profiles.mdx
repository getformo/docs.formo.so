...
| `scope` | string | (Optional) Only for token filters: `any` or `protocol` |
...
Filter by token holdings. Supports an optional `scope` parameter.

#### Token Scope

| Scope | Description |
|-------|-------------|
| `any` (default) | Match token holdings in wallet or any protocol |
| `protocol` | Match only tokens deposited in DeFi protocols |
...
**Example - Find users with USDC deposited in protocols:**
