...
<AccordionGroup>
  <Accordion title="What are the rate limits for the Formo API?">
    | Layer | Limit | Window | Scope | Applied to |
    |-------|-------|--------|-------|------------|
    | API (except Profiles) | 1,000 requests | 15 minutes | Per IP | All `/v0/*` routes except Profiles |
    | Profiles API | 10,000 requests | 15 minutes | Per IP | `/v0/profiles` only |
    | Workspace | 100 requests | 1 second | Per workspace (team) | All authenticated `/v0/*` routes, aggregated across all API keys |
  </Accordion>
