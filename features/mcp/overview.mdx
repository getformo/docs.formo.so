...
<AccordionGroup>
  ...
  <Accordion title="Is my analytics data secure when using the MCP Server?">
    Yes. The MCP Server provides read-only access scoped to your project. Each API key is project-specific, sessions expire after 1 hour of inactivity, and no data is stored by the MCP server itself. You can revoke API keys at any time from your workspace settings.
  </Accordion>
</AccordionGroup>
