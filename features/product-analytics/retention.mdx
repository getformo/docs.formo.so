...
You can select which event to use to calculate week-on-week user retention. This gives product teams better insight into retention rates for specific features and user flows.
...
| **Retention event** | The action that counts as returning in a later week (e.g. `connect`, `transaction`, `page`, or a custom event; "Any event" counts any activity) |
