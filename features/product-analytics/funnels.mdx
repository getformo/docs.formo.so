...
- Step filters apply to every event in the group.
- The chart labels the step with the first event plus the number of alternatives, for example "Swap Initiated or 2 others".
- Via the API, add an `events` array to the step (max 10 entries). The primary `type` and `event` stay the step label and are part of the group:
...
```json
{
  ...
  "events": [
    { "type": "track", "event": "Swap Initiated" },
