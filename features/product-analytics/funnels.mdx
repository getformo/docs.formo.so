...
## Combine Events with OR

<Frame caption="Use OR to count multiple events as one funnel step.">
  <img src="/images/funnel-or-steps.jpg" alt="Funnel step OR group" />
</Frame>

A funnel step can match any event in an OR group. Click **OR** to add up to 10 alternative events; performing any one of them completes the step.

For example, a "Trade Completed" step could match `Swap Completed` OR `Limit Order Filled` OR `Perpetual Position Opened`.

Filters on an event apply only to that event. Filters on the step apply to every event in the group.

Via the API, add the alternatives to the step's `events` array:
...
```json
{
  ...
  "events": [
    ...
    { "type": "track", "event": "Limit Order Filled" },
    { "type": "track", "event": "Perpetual Position Opened" }
  ]
}
```
