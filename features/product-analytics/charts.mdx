...
Use the [Ask AI](/features/product-analytics/ai) feature (the AI assistant) from anywhere on the dashboard to build charts and explore your data. Ask AI to generate queries, explore patterns, surface anomalies, and recommend actions.
...
Charts can also query on-chain data from [Dune](/integrations/dune) instead of your Formo data. Prefix a table with `dune.` (e.g. `dune.ethereum.transactions`) and the chart runs on Dune; otherwise it runs on Tinybird.
...
Dune charts show a **Dune** badge, load in the background (Dune queries can take up to a minute), and need a Dune API key set in **Project Settings → Integrations** before you can save one. Autocomplete switches to Dune (Trino) functions and tables while you're writing a `dune.` query. See the [Dune integration](/integrations/dune) for setup.
