---
title: 'Get Profile'
description: 'Retrieve a comprehensive wallet profile by address, including onchain labels, token holdings, DeFi positions, web demographics, and lifecycle data.'
...
<Tabs>
  <Tab title="Workspace API Key">
    ...
    <Note>
    Create a key with **profiles:read** scope in **Team Settings > API**.
    </Note>
  </Tab>
  <Tab title="x402">
...
| `source` | string | Where the label came from: `formo` for behavioural labels computed from onchain positions, the provider name for attestations (for example `chainalysis`, `coinbase`, `human.tech`), or `import` for labels you added yourself |
