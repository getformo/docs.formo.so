...
Two concurrent requests with the same key in flight return `409 IDEMPOTENCY_IN_PROGRESS`. Wait briefly (typically < 1 s) and replay — the server will return the cached response of the original request.
