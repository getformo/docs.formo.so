...
<Warning>
  **Migrating Query API filters and pagination (breaking).** The analytics endpoints under `/v0/*` (`kpis`, `top_*`, `revenue_*`, `volume_*`, `retention`, `user_*`, `event_timeseries`) recently unified their filter shape and pagination params. The Profiles, Segments, Alerts, and Contracts APIs are unchanged. The Charts API's endpoints and request structure are unchanged, but the `retentionUserFilters` shape inside chart `settings` follows the same `field`/`op` rename described below.
  ...
  - **`metric_column` allowlist.** `revenue_by_metric`, `volume_by_metric`, and `top_sources` now silently return zero rows for `metric_column` values outside their allowlists. The endpoints share an overlapping but not identical set:

    - `revenue_by_metric` and `volume_by_metric` (20 values): `pathname`, `origin`, `channel`, `referrer`, `referrer_url`, `ref`, `utm_source`, `utm_medium`, `utm_campaign`, `utm_content`, `utm_term`, `builder_codes`, `location`, `device`, `browser`, `os`, `rdns`, `provider_name`, `chain_id`, `event`.
    - `top_sources` (13 values): `referrer`, `referrer_url`, `ref`, `utm_source`, `utm_medium`, `utm_campaign`, `utm_term`, `utm_content`, `origin`, `device`, `browser`, `os`, `channel`.

    The empty-result schema for `revenue_by_metric` / `volume_by_metric` is `{ "project_id": "", "metric_value": "", "sum_revenue": 0 }` (or `sum_volume`) — handle that case in clients.
</Warning>
