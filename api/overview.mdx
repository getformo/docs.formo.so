...
- Available via [x402](https://www.x402.org/) (`formo.x402.paysponge.com`) and [MPP](https://mpp.dev/) (`formo.mpp.paysponge.com`): pay per request, no API key required
...
Almost every non-2xx response uses the same envelope (see the exceptions below):
...
<Note>
Two paths do not use this envelope, so treat the status code as authoritative and parse the body defensively:

- **The per-IP rate limiters** (1,000 requests/15 min, and 10,000/15 min on Profiles) reply with a plain-text body. Only the per-workspace limiter (100/second) returns the envelope.
- **The events endpoint** (`POST /v0/raw_events` on `events.formo.so`) returns plain `{ "error": "..." }` objects.
</Note>
