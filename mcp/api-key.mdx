...
4. Enable the permissions you need. **Analytics API** read covers analytics and SQL; add **Alerts**, **Charts**, **Contracts** or **Segments** to let it read or manage those.
...
**`insufficient scope` on a tool call:** the key is missing that tool's permission. Analytics and SQL tools need **Analytics API** read; managing alerts, charts, contracts or segments needs the matching permission. There is no separate MCP scope to enable.
