...
> OAuth is available. See [MCP OAuth](/mcp/oauth) (Recommended).

## Prerequisites

Requires an owner or admin role on the workspace.
...
<Tabs>
  ...
  <Tab title="Cursor">
    ...
    <Note>
    Formo MCP uses the Streamable HTTP transport. Selecting the legacy `sse`
    type makes the connection fail, because the server has no SSE endpoint.
    </Note>
...
<Card title="Example questions" icon="message-question" href="/mcp/overview#example-questions">
