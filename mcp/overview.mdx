...
The Formo MCP server exposes a curated set of tools, grouped below by the same
categories as API key and OAuth permissions. Each tool is authorised against
the matching permission on your API key (or on the OAuth consent you
approved); a connection without it is denied with `insufficient scope`.

**Query API** — SQL, exploration, and pre-built analytics endpoints. Permission: `query:read`.
...
**Profiles API** — wallet enrichment. Permission: `profiles:read`.
...
**Docs** — public Formo documentation; no permission required.
