...
icon: robot
description: 'Connect AI agents, coding tools, and third-party apps to Formo with MCP.'
---
...
| `execute_query` | Run SQL queries against your analytics data (see the note below) |
...
<Note>
`execute_query` is not the same as the REST [Query API](/api/query). The REST endpoint validates your SQL first: one statement, no comments, and a `LIMIT` ceiling. The MCP tool forwards the query to the analytics backend as written, so those checks do not apply and the backend's own limits govern instead. Your query is still scoped to the connected project either way.
</Note>
...
| `execute_saved_chart` | Run a saved chart's query with a chosen date range | `boards:read` |
| `create_board`, `update_board`, `delete_board` | Manage boards | `boards:write` |
| `create_chart`, `update_chart`, `delete_chart`, `duplicate_chart`, `move_chart`, `reorder_charts` | Manage charts | `boards:write` |
...
| `create_contract`, `update_contract`, `delete_contract` | Manage tracked contracts (pipeline inclusion is set via `update_contract`) | `contracts:write` |
...
- **No customer data stored:** the MCP server keeps no copy of your analytics or wallet
  data. It does record request telemetry for each tool call, including the tool name and
  its arguments (truncated at 4,000 characters), so SQL text and any addresses passed as
  arguments are retained in Formo's internal usage analytics.
