...
<Tabs>
  ...
  <Tab icon="star" title="Wagmi (Recommended)">
    #### 1. Install the Formo SDK
    ...
    <Tip>
    **Why Wagmi integration?** When enabled, the SDK hooks directly into Wagmi's state management and TanStack Query's mutation cache. This provides better compatibility with wallet libraries like RainbowKit, ConnectKit, and Web3Modal.
    </Tip>
    ...
    <Warning>
    **Important:** Always provide the `queryClient` to enable full event tracking. Without it, signature and transaction events will not be captured—only connect, disconnect, and chain change events.
    </Warning>
  ...
  <Tab icon="react" title="React">
    ...
    <Note>
    **Using Wagmi?** Use the [Wagmi (Recommended)](#wagmi-recommended) tab instead for better event tracking.
    </Note>
