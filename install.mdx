...
<Accordion title="Copy prompt">
  ...
  **Choose the right method based on the project:**
  ...
  - **Solana** - For Solana apps. Wallet detect, connect, and disconnect are autocaptured through the Wallet Standard with any wallet library.

  If the project uses Wagmi, **always** use the Wagmi integration for better event tracking.

  If the project is a Solana app, **always** use Option E. Wallet connect and disconnect are autocaptured on Solana too. Signatures always need explicit tracking; transactions do too unless a framework-kit store records them.
  ...
  **Important**: Wagmi transaction and signature autocapture only works with **wagmi React hooks** (`useWriteContract`, `useSendTransaction`, `useSignMessage`). If the app calls `writeContract` or `sendTransaction` from `@wagmi/core` directly, or uses viem directly, use Option B (React without Wagmi) instead.
  ...
  Wallet detect, connect, and disconnect are autocaptured through the Wallet Standard for any Solana wallet library (`@solana/wallet-adapter`, framework-kit, Privy, Dynamic, Reown, custom). Do NOT write connect/disconnect glue. Wrap the app with FormoAnalyticsProvider anywhere in the tree:

  <FormoAnalyticsProvider writeKey="<YOUR_WRITE_KEY>" options={{ evm: false }}>

  The Wallet Standard does not say which cluster the app is on, so a devnet or testnet app must name it, or connections are reported on mainnet-beta:

  options={{ evm: false, solana: { cluster: 'devnet' } }}

  If the app uses framework-kit (`@solana/client`), ALSO pass the store so cluster switches and transaction lifecycle events recorded in that store are autocaptured. Framework-kit's React transaction hooks keep their state outside the store and still need explicit tracking:

  options={{ evm: false, solana: { store: client.store } }}

  Solana clusters map to reserved chain IDs (`mainnet-beta` 900001, `testnet` 900002, `devnet` 900003, `localnet` 900004); use the `SOLANA_CHAIN_IDS` constant. Signatures always need explicit `formo.signature()` calls on Solana, and transactions need explicit `formo.transaction()` calls unless a framework-kit store records them. Wallet Standard discovery needs `@formo/analytics` 1.39.0 or later; on an older version, upgrade rather than writing glue.
  ...
  **Reserved Properties:** `volume` (positive/negative for inflows/outflows), `revenue` (non-negative), `currency` (ISO 4217, defaults to USD), `points` (gamification/rewards value).
  ...
  7. **Do not** report wallet connections with `track()`. `formo.track('Connect', ...)` is stored as a custom track event, not the built-in `connect` type, so the Connect wallet funnel step and every wallet connection chart stay empty. Wallet connections are autocaptured on Solana; if you must emit one yourself, use `formo.connect()` / `formo.disconnect()`.
  8. **Do not** write connect/disconnect glue for Solana wallets. Detect, connect, and disconnect are autocaptured through the Wallet Standard on `@formo/analytics` 1.39.0 or later; a second, manual connect double-counts the connection.
...
<Tabs>
  ...
  <Tab icon="circle-nodes" title="Solana">
    ...
    Wallet `detect`, `connect`, and `disconnect` are autocaptured through the [Wallet Standard](https://github.com/wallet-standard/wallet-standard) for every Solana wallet library. Wrap your app with `FormoAnalyticsProvider` anywhere in the tree; the example uses `@solana/wallet-adapter-react`, but nothing in it is specific to that library.
    ...
    <Note>
    Requires `@formo/analytics` 1.39.0 or later. Earlier versions only autocapture Solana wallet events with a framework-kit store.
    </Note>

    <AccordionGroup>
      <Accordion title="Not on mainnet?">
        The Wallet Standard lists the clusters a wallet supports, not the one your app is on, so connections are reported on `mainnet-beta` unless you name the cluster:

        ```tsx
          options={{ evm: false, solana: { cluster: 'devnet' } }}
        ```

        To switch at runtime, call `formo.solana.setCluster('devnet')`; it also emits a `chain` event for the connected wallet.
      </Accordion>
      <Accordion title="framework-kit (store events too)">
        Also pass `client.store` and the SDK captures cluster switches and the lifecycle of transactions recorded in the store. Transactions sent through framework-kit's React hooks still need explicit tracking because those hooks keep their state outside the store.
      ...
      <Accordion title="Opting out of discovery">
        Pass `solana: false` to turn Wallet Standard discovery off, for example for a wallet that does not implement the standard. Nothing is then autocaptured, so emit wallet events yourself with [`connect`](/data/events/connect) and [`disconnect`](/data/events/disconnect). Never do both: a manual connect beside discovery double-counts the connection.
    ...
    Signatures need explicit [`signature`](/data/events/signature) calls, and transactions explicit [`transaction`](/data/events/transaction) calls unless a framework-kit store records them. For app-specific actions, use [`track`](/sdks/web#track-events).
  ...
  <Tab icon="react" title="React">
    ...
    **Building on Solana?** Use the **Solana** tab for Wallet Standard, network, and optional framework-kit setup.
    </Note>
...
<Note>
Solana wallet `detect`, `connect`, and `disconnect` are autocaptured through the Wallet Standard, whichever wallet library your app uses. Signatures always need explicit calls; transactions do too unless a [framework-kit](/sdks/web#solana-with-framework-kit) store records them. See [Solana integration](/sdks/web#solana-integration).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Why are my Solana wallet connections not showing up?">
    Check the SDK version first. Solana wallet connections are autocaptured through the Wallet Standard on `@formo/analytics` 1.39.0 or later, with any wallet library. On earlier versions they were only autocaptured with a [framework-kit](/sdks/web#solana-with-framework-kit) store, and every other app had to call [`connect()`](/data/events/connect) and [`disconnect()`](/data/events/disconnect) itself. Upgrading is the fix. If you passed `solana: false`, discovery is off and you emit them yourself; see [Solana manual wallet events](/sdks/web#solana-manual-wallet-events).

    Connections are reported on `mainnet-beta` unless you set `solana.cluster`, so a devnet app that filters by chain will find them under the wrong cluster rather than missing.
