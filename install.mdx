...
<AccordionGroup>
  ...
  <Accordion title="Why are my transactions and signatures not being tracked?">
    Transaction and signature tracking requires either the Wagmi integration or manually calling [formo.transaction()](/data/events/transaction) and [formo.signature()](/data/events/signature). The HTML snippet alone tracks pageviews, sessions, referrer sources, and UTM parameters — but not wallet transactions or signatures. Make sure you have installed the SDK with one of the framework integrations that support wallet event tracking.
  </Accordion>
  ...
  <Accordion title="Does the Formo SDK work on localhost during development?">
    Yes. The SDK works on localhost and all domains by default. Events from localhost will appear in your dashboard, so you may want to use a separate project for development and production.
  </Accordion>
  <Accordion title="How do I prevent ad blockers from blocking Formo?">
    Set up a [reverse proxy](/sdks/web#proxy) to route Formo requests through your own domain. This prevents ad blockers from intercepting requests to `api.formo.so`.
  </Accordion>
</AccordionGroup>
