...
<Tabs>
  ...
  <Tab icon="circle-nodes" title="Solana">
    ...
    [Wallet Standard](https://github.com/wallet-standard/wallet-standard) autocaptures `detect`, `connect`, and `disconnect` regardless of wallet library. Wrap your app with `FormoAnalyticsProvider`:
    ...
    <AccordionGroup>
      ...
      <Accordion title="Opting out of discovery">
        Pass `solana: false` to disable discovery, then emit [`connect`](/data/events/connect) and [`disconnect`](/data/events/disconnect) yourself. Do not combine discovery with manual wallet events.
...
<AccordionGroup>
  ...
  <Accordion title="Why are my Solana wallet connections not showing up?">
    Use `@formo/analytics` 1.39.0 or later. Earlier versions only autocapture Solana connections from a [framework-kit](/sdks/web#solana-with-framework-kit) store. Also check that discovery is enabled and `solana.cluster` matches your chain. See [Solana manual wallet events](/sdks/web#solana-manual-wallet-events).
