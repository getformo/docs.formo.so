...
<Accordion title="Copy prompt">
  ...
  **Choose the right method based on the project:**
  ...
  - **Solana** - For Solana apps. Wallet events are autocaptured through the Wallet Standard with any wallet library.
  ...
  **Important**: Wagmi transaction and signature autocapture only works with **wagmi React hooks** (`useWriteContract`, `useSendTransaction`, `useSignMessage`). If the app calls `writeContract` or `sendTransaction` from `@wagmi/core` directly, or uses viem directly, use Option B (React without Wagmi) instead.
  ...
  Wallet detect, connect, and disconnect are autocaptured through the Wallet Standard for EVERY Solana wallet library (`@solana/wallet-adapter`, framework-kit, Privy, Dynamic, Reown, custom). Do NOT write connect/disconnect glue. Wrap the app with FormoAnalyticsProvider anywhere in the tree:
  ...
  **Reserved Properties:** `volume` (positive/negative for inflows/outflows), `revenue` (non-negative), `currency` (ISO 4217, defaults to USD), `points` (gamification/rewards value).
  ...
  8. **Do not** write connect/disconnect glue for Solana wallets. Wallet events are autocaptured through the Wallet Standard on `@formo/analytics` 1.39.0 or later; a second, manual connect double-counts the connection.
...
<Tabs>
  ...
  <Tab icon="react" title="React">
    ...
    **Building on Solana?** Use the **Solana** tab instead for cluster and framework-kit setup.
    </Note>
