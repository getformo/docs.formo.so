...
<Tip>
Use this pre-built prompt to install faster: [Open in Cursor](https://cursor.com/link/prompt?text=Install%20Formo%20Analytics%20using%20the%20instructions%20at%20https%3A%2F%2Fdocs.formo.so%2Finstall%20and%20https%3A%2F%2Fdocs.formo.so%2Fsdks%2Fweb.%20Detect%20the%20framework%20and%20follow%20the%20matching%20setup.%20For%20Solana%20dapps%2C%20prefer%20framework-kit%20and%20pass%20options.solana.store.%20Formo%20autocaptures%20compatible%20Wallet%20Standard%20detect%2C%20connect%2C%20and%20disconnect%20events%2C%20cluster%20changes%2C%20and%20transaction%20lifecycle%20events%20recorded%20in%20the%20store.%20Set%20options.solana.cluster%20for%20non-mainnet%20apps%20without%20a%20store.%20Use%20the%20Solana%20Kit%20setup%20when%20the%20app%20does%20not%20use%20framework-kit.%20Track%20signatures%20and%20transactions%20not%20recorded%20in%20framework-kit%20explicitly.%20After%20setup%2C%20identify%20connected%20users%20and%20suggest%20relevant%20custom%20events.)
</Tip>
...
<Accordion title="Copy prompt">
  ...
  **Choose the right method based on the project:**
  ...
  - **framework-kit** - Recommended for Solana dapps; pass its store for the fullest autocapture.

  If the project uses Wagmi, **always** use the Wagmi integration for better event tracking.

  For Solana dapps, prefer framework-kit and pass its store. Solana Kit and other Wallet Standard integrations still autocapture wallet detection and connections without a store.
  ...
  **Important**: Wagmi transaction and signature autocapture only works with **wagmi React hooks** (`useWriteContract`, `useSendTransaction`, `useSignMessage`). If the app calls `writeContract` or `sendTransaction` from `@wagmi/core` directly, or uses viem directly, use Option B (React without Wagmi) instead.
  ...
  Prefer framework-kit for Solana dapps. Pass its store to autocapture wallet events, cluster switches, and transaction lifecycle events recorded in the store:

  <FormoAnalyticsProvider writeKey="<YOUR_WRITE_KEY>" options={{ evm: false, solana: { store: client.store } }}>

  If the app uses Solana Kit or another Wallet Standard integration, no store or connection glue is required. Set non-mainnet clusters explicitly:

  options={{ evm: false, solana: { cluster: 'devnet' } }}

  Track signatures and any transactions not recorded in a framework-kit store explicitly.
...
<Tabs>
  ...
  <Tab icon="circle-nodes" title="Solana">
    ...
    framework-kit is recommended for Solana dapps. Its store adds cluster and recorded transaction lifecycle events to the wallet events Formo autocaptures through [Wallet Standard](https://github.com/wallet-standard/wallet-standard).

    <AccordionGroup>
      <Accordion title="framework-kit (Recommended)">
        Pass `client.store` for cluster switches and transaction lifecycle events recorded in the store.
    ...
    Track signatures and transactions that are not recorded in a [framework-kit](/sdks/web#solana-with-framework-kit) store explicitly. Use [`track`](/sdks/web#track-events) for app-specific actions.
