...
<Accordion title="Copy prompt">
  ...
  **Choose the right method based on the project:**
  ...
  - **Solana** - Wallet Standard autocaptures detect, connect, and disconnect.

  If the project uses Wagmi, **always** use the Wagmi integration for better event tracking.

  If the project is a Solana app, **always** use the Solana option. Transaction autocapture is supported if you use framework-kit.
  ...
  **Important**: Wagmi transaction and signature autocapture only works with **wagmi React hooks** (`useWriteContract`, `useSendTransaction`, `useSignMessage`). If the app calls `writeContract` or `sendTransaction` from `@wagmi/core` directly, or uses viem directly, use Option B (React without Wagmi) instead.
  ...
  Wallet Standard autocaptures detect, connect, and disconnect regardless of wallet library. Do NOT add connect/disconnect glue:
  ...
  With framework-kit, pass the store to add cluster switches and transactions recorded there. React hook transactions still need explicit tracking:

  options={{ evm: false, solana: { store: client.store } }}

  Use `SOLANA_CHAIN_IDS` for chain IDs. Signatures and transactions outside a framework-kit store need explicit tracking. Wallet Standard discovery requires `@formo/analytics` 1.39.0 or later.
  ...
  **Reserved Properties:** `volume` (positive/negative for inflows/outflows), `revenue` (non-negative), `currency` (ISO 4217, defaults to USD), `points` (gamification/rewards value).
  ...
  8. **Do not** add Solana connect/disconnect glue on `@formo/analytics` 1.39.0 or later. Wallet Standard already autocaptures it.
...
<Tabs>
  ...
  <Tab icon="circle-nodes" title="Solana">
    ...
    We recommend framework-kit for the fullest autocapture. With other libraries, compatible wallets get `detect`, `connect`, and `disconnect` through [Wallet Standard](https://github.com/wallet-standard/wallet-standard).

    <Note>
    Requires `@formo/analytics` 1.39.0 or later. Earlier versions only autocapture Solana connections from a framework-kit store.
    </Note>

    <AccordionGroup>
      <Accordion title="framework-kit (Recommended)">
        Pass `client.store` to add cluster switches and transactions recorded in the store. Hook-based transactions still need explicit tracking.
      ...
      <Accordion title="Other wallet libraries">
        Wrap your existing wallet provider with `FormoAnalyticsProvider`:
    ...
    Transaction autocapture is supported if you use [framework-kit](/sdks/web#solana-with-framework-kit). Use [`track`](/sdks/web#track-events) for app-specific actions.
...
<Note>
Wallet Standard autocaptures Solana `detect`, `connect`, and `disconnect`. Signatures and transactions outside a [framework-kit](/sdks/web#solana-with-framework-kit) store need explicit tracking. See [Solana integration](/sdks/web#solana-integration).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Why are my Solana wallet connections not showing up?">
    Use `@formo/analytics` 1.39.0 or later. Earlier versions only autocapture Solana connections from a [framework-kit](/sdks/web#solana-with-framework-kit) store. Also check that `solana.cluster` matches your chain. If the wallet does not implement Wallet Standard, use the [manual connection fallback](/sdks/web#wallets-without-wallet-standard).
