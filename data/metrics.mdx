...
Funnel
Funnels are essentially multi-stage conversion targets. 
Each step has it's own filter that calculates the visitor drop off and the conversion rate (CR). 
The main difference between regular CR and funnel CR is that funnel CR is always based on the previous step, not the total number of visitors.