...
The SDK sends the full user-agent string (`navigator.userAgent`) and a classified browser type (e.g. `chrome`, `firefox`, `safari`). Both are stored.
...
- If the Formo dashboard displays ENS names or social handles, this data is resolved from **public on-chain sources** (e.g. ENS registry, Farcaster) — not collected by the SDK from your users
- We do not correlate wallet addresses with off-chain personal data, through probabilistic matching or any other means.

<Info>
Public blockchain data (wallet addresses, ENS names, onchain social profiles) is already visible to anyone via block explorers like Etherscan and the ENS protocol. Formo does not create new data linkages, it surfaces data that is already public for accessibility.
</Info>
