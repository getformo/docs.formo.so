...
To count user sessions, we generate a daily changing identifier using the visitor’s IP address and User-Agent. Every single HTTP request sends this information to the server so that’s what we use. 
