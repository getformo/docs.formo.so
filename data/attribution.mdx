...
- The user visits the app's website (fooswap.com) (*"session start"*)
...
sequenceDiagram
    ...
    note over User,App: Detect timezone, device, os, UTMs, referrers
        activate Formo    


    %% Session start
    User->>Website: visit (via links / ads / campaigns)
Website->>App: internal site navigation
    Website->>Formo: record session start and offchain context
        App->>Formo: record session events

note over Website,Wallet: Detect user identity, wallet address, rdns provider
note over App,Formo: Index transactions, wallets, contracts

    User->>Wallet: connect wallet & sign transactions

    App->>Chain: contract calls (e.g. swap, mint, stake)

    App->>Formo: record transaction
    Chain-->>App: return tx status & hash    
    ...
    activate Chain
    Chain-->>Formo: record tx receipt & events
    deactivate Chain

    Formo->>Formo: match events with context
    Formo->>Formo: calculate attribution

    Formo-->>App: unified identity and attribution for onchain apps
    Formo-->>User: wallet-aware personalized experiences
    deactivate Formo
```
